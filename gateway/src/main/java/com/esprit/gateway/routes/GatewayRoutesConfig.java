    ...
    return route("employee-management")
        .route(
            path(
                "/api/employees/**",
                "/api/staff/**",
                "/employee-management/v3/api-docs",
                "/api/iam/**"),
            http())
        ...
        .filter(
            circuitBreaker(
                "employee-management", URI.create("forward:/fallback/employee-management")))
        .build()
        .and(
            route("menu-management")
                ...
                .filter(
                    circuitBreaker(
                        "menu-management", URI.create("forward:/fallback/menu-management")))
                .build())
        .and(
            route("delivery-management")
                .route(
                    path(
                        "/api/deliveries/**", "/api/orders/**", "/delivery-management/v3/api-docs"),
                    http())
                .before(rewritePath("/delivery-management/v3/api-docs", "/v3/api-docs"))
                .filter(lb("delivery-management"))
                .filter(
                    circuitBreaker(
                        "delivery-management", URI.create("forward:/fallback/delivery-management")))
                .build())
        .and(
            route("reservation")
                .route(
