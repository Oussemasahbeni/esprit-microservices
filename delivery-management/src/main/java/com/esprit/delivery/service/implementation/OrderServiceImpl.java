...
public class OrderServiceImpl implements OrderService {
  ...
  static {
    ...
    ALLOWED_TRANSITIONS.put(
        OrderStatus.READY_FOR_PICKUP, EnumSet.of(OrderStatus.PICKED_UP, OrderStatus.CANCELLED));
  ...
  public Order placeOrder(CreateOrderRequest request) {
    ...
    Customer customer = customerService.getOrCreateCustomer(request.getCustomerId());
    ...
    if (order.getDeliveryAddress() == null) {
      throw new ApplicationException(
          INVALID_ORDER, "No delivery address provided and customer has no default on file");
    }
    ...
    for (CreateOrderRequest.OrderItemRequest itemRequest : request.getItems()) {
      if (itemRequest.getQuantity() == null || itemRequest.getQuantity() <= 0) {
        throw new ApplicationException(
            INVALID_ORDER, "Quantity must be positive for dish " + itemRequest.getDishId());
      }
      ...
      if (dish == null || !dish.isAvailable()) {
        throw new ApplicationException(
            UNAVAILABLE_DISH, "Dish not found: " + itemRequest.getDishId());
      }
    ...
    customerService.recordOrderPlaced(saved.getCustomerId());

    eventPublisher.publishOrderPlaced(toOrderPlacedEvent(saved));
    log.info(
        "Order {} placed for customer {} (total={})",
        saved.getId(),
        saved.getCustomerId(),
  ...
  public Order getOrderById(Long orderId) {
    return orderRepository
        .findByIdFetchItems(orderId)
        .orElseThrow(
            () -> new ApplicationException(ORDER_NOT_FOUND, "Order not found with id: " + orderId));
  }
  ...
  public List<Order> getOrdersByStatus(OrderStatus status) {
    return status != null
        ? orderRepository.findByStatusFetchItems(status)
        : orderRepository.findAllFetchItems();
  }
  ...
  public Order updateOrderStatus(Long orderId, UpdateOrderStatusRequest request) {
    ...
    if (newStatus == null) {
      throw new ApplicationException(INVALID_ORDER, "newStatus is required");
    }

    Set<OrderStatus> allowedNextStatuses =
        ALLOWED_TRANSITIONS.getOrDefault(previousStatus, EnumSet.noneOf(OrderStatus.class));
    if (!allowedNextStatuses.contains(newStatus)) {
      throw new ApplicationException(
          INVALID_ORDER_STATUS_TRANSITION,
          String.format(
              "Invalid order status transition from %s to %s", previousStatus, newStatus));
    }
  ...
  public void handleDishUnavailable(Long dishId) {
    List<Order> affectedOrders =
        orderRepository.findNonTerminalOrdersContainingDish(dishId, TERMINAL_STATUSES);
    ...
    log.warn(
        "Dish {} became unavailable; flagging {} affected order(s) for review",
        dishId,
        affectedOrders.size());

    for (Order order : affectedOrders) {
      eventPublisher.publishOrderFlaggedForReview(
          OrderFlaggedForReviewEvent.builder()
              .orderId(order.getId())
